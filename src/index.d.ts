...
> = 
    string extends P | S ? never // don't consider infinite types
  : P extends "" ? never // the prefix literal should be nonempty
  : S extends `${P}${infer R extends string}` ? R
  : never;

type Opt<P extends string, S extends string> = 
    [Derivative<P, S>] extends [never] ? S
  : Derivative<P, S>;

type RecognizeDerivative<P extends string, S extends string> = Eq<Derivative<P, S>, "">;
...
export type DerivePattern<RE extends string, S extends string> =
  ...
  : RE extends "" ? S
  : S extends "" ? never
  : RE extends `[${infer P}]*${infer Rest}` ? DerivePattern<Rest, Many0<CharClass<P>, S>>
  : RE extends `[${infer P}]+${infer Rest}` ? DerivePattern<Rest, Many1<CharClass<P>, S>>
  : RE extends `[${infer P}]?${infer Rest}` ? DerivePattern<Rest, Opt<CharClass<P>, S>>
